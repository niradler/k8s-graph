...
class IstioHandler(BaseCRDHandler):
    def supports(self, resource: dict[str, Any]) -> bool:
        ...
        return (
            kind in ["VirtualService", "DestinationRule", "Gateway", "ServiceEntry"] and
            "istio.io" in api_version
        )
                ...
                for route in http_routes:
                    for destination in route.get("route", []):
                        ...
                        if host and "." not in host:
                            ...
                            service_namespace = host.split(".")[1] if len(host.split(".")) > 1 else namespace
