...
class HelmHandler(BaseCRDHandler):
    def supports(self, resource: dict[str, Any]) -> bool:
        ...
        return (
            labels.get("meta.helm.sh/release-name") is not None
            or resource.get("metadata", {}).get("labels", {}).get("app.kubernetes.io/managed-by")
            == "Helm"
        )
    ...
    async def discover(self, resource: dict[str, Any]) -> list[ResourceRelationship]:
        ...
        try:
            ...
            release_name = annotations.get("meta.helm.sh/release-name") or labels.get(
                ...
                try:
                    ...
                    for secret in secrets:
                        ...
                        if secret_name.startswith(release_secret_name) or (
                            secret_labels.get("owner") == "helm"
                            and secret_labels.get("name") == release_name
                        ):
                ...
                for kind in [
                    ...
                    try:
                        ...
                        for res in resources:
                            ...
                            res_release = res_annotations.get(
                                "meta.helm.sh/release-name"
                            ) or res_labels.get("app.kubernetes.io/instance")

                            if res_release == release_name and res_metadata.get(
                                "name"
                            ) != metadata.get("name"):
                                relationships.append(
                                    ResourceRelationship(
