...
from k8s_graph import BuildOptions, DiscoveryOptions, GraphBuilder, ResourceIdentifier
...
class TestIncludeCRDsFlag:
    ...
    def test_crd_handlers_have_crd_category(self, registry):
        ...
        for handler in get_all_handlers():
            assert (
                handler.categories & DiscovererCategory.CRD
    ...
    async def test_include_crds_false_filters_handlers(self, mock_client, registry):
        ...
        assert len(relationships_with) >= len(
            relationships_without
    ...
    async def test_multiple_crd_types_in_namespace(self, mock_client):
        ...
        def get_resource_side_effect(resource_id):
            for res in resources:
                if (
                    res["kind"] == resource_id.kind
                    and res["metadata"]["name"] == resource_id.name
                    and res["metadata"].get("namespace") == resource_id.namespace
                ):
                    return res
            return None
